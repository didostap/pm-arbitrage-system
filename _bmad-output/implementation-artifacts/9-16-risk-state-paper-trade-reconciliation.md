...
- `validatePosition` and `getCurrentExposure` remain live-only by design — mode separation occurs at `reserveBudget`/`commitReservation` level. Dashboard per-mode capital uses direct DB queries via `computeModeCapital`.
