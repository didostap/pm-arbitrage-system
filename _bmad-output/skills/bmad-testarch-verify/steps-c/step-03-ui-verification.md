...
```
mcp__playwright-test__browser_navigate(url: "{app_url}")
```
...
### 7. Close Browser (if last UI step)

Only close the browser if no further UI interaction is needed:
```
mcp__playwright-test__browser_close()
```

If data verification (step 04) might need the browser for UI-triggered operations, keep it open.

### 8. Update Report Frontmatter

Update the report file:
- Add `"ui-verification"` to `stepsCompleted` array
- Update `lastStep` to `"ui-verification"`
